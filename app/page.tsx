...
export default function Home() {
  ...
  return <>
    ...
    <section className="hero"><span className="hero-decoration hero-decoration-one" /><span className="hero-decoration hero-decoration-two" /><div className="container hero-grid">
      <div className="hero-copy"><p className="eyebrow">Education technology for every learning ecosystem</p><h1>Powering the <span>Future of Education</span></h1><p>Edora Learning provides one intelligent, secure and adaptable platform for publishers, schools, coaching institutes, academies, educators and learners.</p><div className="button-row"><Link className="button button-primary" href="/contact?interest=demo">Request a Demo <ArrowRight size={18} /></Link><Link className="button button-secondary" href="/platform">Explore Edora Education OS</Link></div><div className="hero-trust"><ShieldCheck aria-hidden="true" /><span>Secure by design</span><span>•</span><span>Built for every education role</span></div></div>
    ...
    <section className="section section-soft-blue audience-section"><div className="container"><SectionHeading eyebrow="Who we serve" title="Built for the Complete Education Ecosystem" description="Purposeful experiences for every organization and person involved in learning." /><div className="grid-3 audience-grid">
      <SolutionCard icon={BookOpen} title="Publishers" description="Digitize books, resources, school relationships and AI-powered content." href="/solutions#publishers" linkLabel="Explore Publisher Solutions" />
      <SolutionCard icon={School} title="Schools" description="Manage academics, educators, students and learning from one place." href="/solutions#schools" linkLabel="Explore School Solutions" />
      <SolutionCard icon={Target} title="Coaching Institutes" description="Organize courses, batches, faculty, assessments and online learning." href="/solutions#coaching" linkLabel="Explore Coaching Solutions" />
      <SolutionCard icon={Building2} title="Academies" description="Launch branded learning programs with mentors, AI and analytics." href="/solutions#academies" linkLabel="Explore Academy Solutions" />
      <SolutionCard icon={UserRound} title="Educators" description="Access approved resources, teaching tools and intelligent workflows." href="/solutions#educators" linkLabel="Explore Educator Solutions" />
      <SolutionCard icon={Users} title="Learners and Families" description="Learn, practise, assess progress and receive personalized support." href="/solutions#learners" linkLabel="Explore Learner Solutions" />
    </div></div></section>

    <section className="section section-eeos"><div className="container split"><div><SectionHeading eyebrow="The platform" title="Meet Edora Education OS" description="A configurable Education Operating System designed for publishers, institutions and learning organizations." /><ul className="check-list">{["One secure platform", "Independent branded tenants", "Simple role-based experiences", "Built for growth", "Configurable features", "Cloud-ready architecture"].map((item) => <li key={item}><CheckCircle2 size={20} />{item}</li>)}</ul><div className="button-row"><Link className="button button-primary" href="/platform">Explore the Platform <ArrowRight size={18} /></Link></div></div><div className="stack-visual" aria-label="Platform capability stack">{platformLayers.map(([Icon, title, description]) => <div className="stack-row" key={title}><span><Icon size={20} /></span><div><strong>{title}</strong><small>{description}</small></div></div>)}</div></div></section>
