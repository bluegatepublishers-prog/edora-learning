...
export default function DemoPage() {
  return <>
    <section className="demo-hero"><span className="demo-hero-shape demo-shape-one" /><span className="demo-hero-shape demo-shape-two" /><div className="container demo-hero-inner"><div><p className="eyebrow">Explore Edora Education OS</p><h1>See the Complete Education Ecosystem <span>in Action</span></h1><p>Explore how Edora connects publishers, institutions, educators, learners, mentors and families through one secure and intelligent platform.</p><div className="button-row"><Link className="button button-primary" href="/contact?interest=demo">Request a Live Demo <ArrowRight size={18} /></Link><Link className="button button-outline-light" href="/contact?interest=platform">Talk to Our Team</Link></div></div><div className="demo-hero-visual" aria-label="Connected Edora platform roles"><div className="demo-orbit"><span><ShieldCheck aria-hidden="true" />Secure platform</span>{[BookOpen, School, UserRound, GraduationCap, Users].map((Icon, index) => <i key={index}><Icon aria-hidden="true" /></i>)}</div></div></div></section>
    ...
    <section className="demo-final-cta"><div className="container"><div><p className="eyebrow">Guided platform walkthrough</p><h2>Ready to Explore Edora Education OS?</h2><p>Book a guided demonstration tailored to your organization.</p><div className="demo-contact-line"><a href={`mailto:${siteConfig.email}`}>{siteConfig.email}</a><span>•</span><a href={`tel:${siteConfig.phoneHref}`}>{siteConfig.phone}</a></div></div><div className="button-row"><Link className="button button-primary" href="/contact?interest=demo">Request a Live Demo</Link><Link className="button button-secondary" href="/contact?interest=platform">Discuss Your Requirements</Link></div></div></section>
  </>;
}
