...
export default function PartnersPage() { return <><PageHero eyebrow="Partner with Edora" title="Build Better Learning Experiences Together" description="Partner with a focused education technology company that understands content, institutions, educators and learners." accent="yellow" /><section className="section section-soft-blue"><div className="container"><SectionHeading eyebrow="Partnership pathways" title="A Shared Foundation for Meaningful Education Work" description="Each partnership begins with a clear need, an appropriate scope and a practical path to value." /><div className="grid-2 partner-grid">{partners.map(({ icon: Icon, title, description, items }) => <article className="partner-card" key={title}><span className="partner-icon"><Icon aria-hidden="true" /></span><h3>{title}</h3><p>{description}</p><ul>{items.map((item) => <li key={item}>{item}</li>)}</ul></article>)}</div></div></section><CTASection title="Become an Edora Partner" description="Partnerships are shaped around the organization, communities served and the education experience being developed." /></> }
